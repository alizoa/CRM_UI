...
export function AttentionSignals({ loading, errorMessage, openTaskCount, overdueTaskCount, signals }: AttentionSignalsProps) {
  return (
    <aside className="rounded border border-gray-200 bg-white p-5">
      ...
      {loading ? <p className="mt-5 rounded border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">Checking customer signals...</p> : null}
      ...
      {!loading && !errorMessage && signals.length === 0 ? (
        <p className="mt-5 rounded border border-green-200 bg-green-50 p-4 text-sm text-green-800">Customer looks organized.</p>
      ) : null}
      {!loading && !errorMessage && signals.length > 0 ? (
